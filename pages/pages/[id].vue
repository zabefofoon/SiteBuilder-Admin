  ...
  <NuxtLayout name="default">
    <div class="flex gap-4 h-full">
      <div class="flex flex-col gap-2">
        <button
          class="w-fit | p-2 | flex | rounded-lg | hover:text-white | hover:bg-black | border"
          title="Config page"
      ...
      <ConfigPage v-if="page && isConfig" :page="page" />
      <Editor v-if="page && isEditor" :page="page" />
    </div>
  </NuxtLayout>
</template>
...
const isConfig = computed(
  () => route.query.selectedTab === 'config' || !route.query.selectedTab
)
